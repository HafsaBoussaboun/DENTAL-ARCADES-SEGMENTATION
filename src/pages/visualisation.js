...
import style from './vtk/GeometryViewer.module.css';
...
//import vtkURLExtract from '@kitware/vtk.js/Common/Core/URLExtract'; // Import d'autres dépendances si nécessaires
...
function createPipeline(fileName, fileContents) {
  ...
  // Créer un bouton avec createElement
  const boutonRetour = document.createElement('button');
  boutonRetour.textContent = 'Retour à la page d\'accueil';
  boutonRetour.addEventListener('click', retourPageAccueil);
  // Ajouter du style au bouton
  boutonRetour.style.position = 'fixed'; // Position fixe pour rester en haut de la page
  boutonRetour.style.top = '10px'; // Position en haut de la page (10px depuis le haut)
  boutonRetour.style.right = '10px'; // Position à droite de la page (10px depuis la droite)
  boutonRetour.style.padding = '10px 20px'; // Définir la marge intérieure (padding)
  boutonRetour.style.backgroundColor = '#007bff'; // Définir la couleur de fond
  boutonRetour.style.color = '#fff'; // Définir la couleur du texte
  boutonRetour.style.border = 'none'; // Supprimer la bordure
  boutonRetour.style.cursor = 'pointer'; // Définir le curseur
  boutonRetour.style.borderRadius = '5px'; // Définir un coin arrondi
  rootControllerContainer.appendChild(boutonRetour);
